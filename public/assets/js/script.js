$(document).ready(function(){
    ...
    $("#uploadFile1").on("change", function() {
        ...
        if (/^image/.test(files[0].type)) { // only image file
            ...
            reader.onloadend = function () { // set image data as background of div
                $("#sectionPhotoEvent1").css("background-image", "url(" + this.result + ")");
            }
        }
    });
    ...
    $("#uploadFile2").on("change", function() {
        ...
        if (/^image/.test(files[0].type)) { // only image file
            ...
            reader.onloadend = function () { // set image data as background of div
                $("#sectionPhotoEvent2").css("background-image", "url(" + this.result + ")");
            }
        }
    });
    ...
    $("#uploadFile3").on("change", function() {
        ...
        if (/^image/.test(files[0].type)) { // only image file
            ...
            reader.onloadend = function () { // set image data as background of div
                $("#sectionPhotoEvent3").css("background-image", "url(" + this.result + ")");
            }
        }
    });
